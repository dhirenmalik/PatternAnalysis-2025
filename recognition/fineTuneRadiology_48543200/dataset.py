...
# These are common column names; we’ll try to auto-detect if user doesn’t pass them.
CANDIDATE_INPUT_KEYS = ("expert_report", "report", "radiology_report", "source")
CANDIDATE_TARGET_KEYS = ("lay_summary", "summary", "target")
...
def build_tokenized(
    ...
    def preprocess(batch):
        ...
        model_inputs = tokenizer(
            sources, max_length=max_input_len, truncation=True, padding="max_length"
        )
        labels = tokenizer(
            batch[target_col], max_length=max_target_len, truncation=True, padding="max_length"
        )
