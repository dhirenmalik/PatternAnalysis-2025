...
def set_seed(seed: int) -> None:
    ...
    Returns:
        None. Seeds are applied as a side effect.
    """
    random.seed(seed)
    np.random.seed(seed)
    ...
    def __init__(
        ...
        *,
        ...
        Args:
            ...
            total_training_steps: Planned number of optimiser steps; used for schedulers.
            scheduler_type: Optional scheduler identifier understood by ``transformers``.
            warmup_ratio: Fraction of steps reserved for scheduler warmup.

        Returns:
            None. Initialises trainer attributes in place.
        """
...
def save_training_artifacts(
    ...
    *,
    ...
    Args:
        ...
        use_lora: Whether LoRA adapters were active during training.

    Returns:
        None. Generates structured artefacts under ``output_dir``.
    """
...
def save_training_plots(output_dir: Path, results: Dict[str, Any]) -> None:
    """Persist loss, ROUGE, and learning-rate curves.

    Args:
        output_dir: Target directory where plot images will be stored.
        results: Training summary dictionary emitted by :meth:`ManualTrainer.train`.

    Returns:
        None. Plots are written to disk when matplotlib is available.
    """
    if not HAS_MPL:
...
def main():
    ...
    Returns:
        None. Drives the training workflow and writes artefacts to disk.
    """
    args = parse_args()
    ...
    def detect_device() -> torch.device:
        """Select the most capable available device for training.

        Returns:
            Torch device prioritising CUDA, then MPS, then CPU.
        """
        if torch.cuda.is_available():
